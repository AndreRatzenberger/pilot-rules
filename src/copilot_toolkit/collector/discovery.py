...
def collect_files(sources: List[Dict[str, Any]], excludes: List[Dict[str, Any]]) -> Tuple[List[str], Set[str]]:
    ...
    """
    console.print(Panel.fit("[bold blue]Collecting Files[/bold blue]", border_style="blue"))
    ...
    with Progress(
        ...
        source_task = progress.add_task("[yellow]Processing sources...", total=len(sources))
        ...
        for source in sources:
            ...
            progress.update(source_task, description=f"[yellow]Scanning: [cyan]{root_path}[/cyan] for [green]{ext_display}[/green]")

            if not root_path.is_dir():
                console.print(f"[yellow]⚠ Warning:[/yellow] Source root '[cyan]{root_path}[/cyan]' is not a directory. Skipping.")
            ...
            if extensions == ['*']:
                ...
                try:
                    ...
                    console.print(f"[yellow]⚠ Warning:[/yellow] Error during globbing for '[cyan]{glob_pattern_str}[/cyan]': [red]{e}[/red]")
            else:
            ...
            console.print(f"  Found [green]{len(found_in_source)}[/green] potential files in this source.")
            all_found_files.update(found_in_source)
            progress.update(source_task, advance=1)

    console.print(f"Total unique files found before exclusion: [bold green]{len(all_found_files)}[/bold green]")

    # Apply exclusion rules
    excluded_files: Set[str] = set()
    if excludes:
        console.print(Panel.fit("[bold yellow]Applying Exclusion Rules[/bold yellow]", border_style="yellow"))
        ...
        with Progress(
            ...
            exclude_task = progress.add_task("[yellow]Processing exclusion rules...", total=len(excludes))
            ...
            for rule in excludes:
                ...
                progress.update(exclude_task, description=f"[yellow]Excluding: [green]{ext_display}[/green] matching [cyan]*{rule_pattern}*[/cyan]")
    ...
    console.print(f"[bold green]✓[/bold green] Collection complete! Found [bold green]{len(final_files)}[/bold green] files with [bold cyan]{len(final_extensions)}[/bold cyan] unique extensions.")
