...
def get_file_metadata(file_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            console.print(f"[yellow]⚠ Warning:[/yellow] Could not read content/count lines for [cyan]{file_path}[/cyan]: [red]{read_err}[/red]")
            metadata["line_count"] = 0 # Indicate unreadable or binary

    except Exception as e:
        console.print(f"[yellow]⚠ Warning:[/yellow] Could not get complete metadata for [cyan]{file_path}[/cyan]: [red]{e}[/red]")
...
def print_header(title: str, style: str = "blue") -> None:
    ...
    console.rule()
    console.print(Panel.fit(f"[bold {style}]{title}[/bold {style}]", border_style=style))
